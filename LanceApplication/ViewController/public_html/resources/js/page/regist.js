$(function(){
    ...
    var check_company = function(){
     ...
     var check_vcode = function(){
        var vcode = $("#inp_verCode");
        var mck = function(){
            if(ckresult.vcode == false)
                vcode.blur();
         };
        ...
        vcode.blur(function(){
         ...
         var vcodeAjax = function(obj){
            obj.attr("disabled", true);
            $.ax("get", "user/check/" + obj.val(), null, function(data){
                if(data == false){
                    obj.attr("data-content", "验证码错误!").popover("show");
                    obj.closest(".form-group").addClass("has-error");
                }else{
                    obj.closest(".form-group").addClass("has-success").addClass("has-feedback");
                    ckresult.vcode = true;
...
function toLogin(uname,password,code,btn){
  var param={
        name : uname,
        pass : password,
        vcode:code,
