...
$.ae = function (err, callback) {
	var option = {
		title : "提示", content : err, buttons : ["确定", "取消"], showBut : true
	};
...
$.ax = function (ptype, path, param, callback, errCallback, jtype) {
	...
	$.ajax( {
		type : ptype, url : BASEURL + path, data : param, dataType : jtype, timeout : 60000, contentType : 'application/json', success : function (data) {
			callback(data);
		},
...
var Lancer = {
	profile : 　
	{
		...
		getEmailPush : function (callback, infor) {
			if (Data != null) {
				callback(Data.User);
			} else {
				$.ax("get", "user/" + User.UserName, null, function (data) {
